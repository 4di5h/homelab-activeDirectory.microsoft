...
Param
    (
        ...
        # Set DNS Address of the Domain Controller - must be the IP address of the Domain Controller
        [string]$dnsAddress = '10.10.10.14',

        # Set Hostname for this host
        [string]$hostname = 'ClientVM',

        # Set domain name for this host to join to
        [string]$domainName = 'homelab.com',

        # Set account username with permissions to add host to domain
        [string]$username = "DomainClientAccount",

        # Set password of above account
        [string]$password = "samplep@ssword123"
        
    )

# Get the name of this script - this command only works if you run the script file i.e. not from powershell_ise.exe
$scriptName = 'setup-client'
...
Function Configure-IPandDNS
    {
        ...
        Update-Log -logMessage "Changing with new IP address: $new_IPv4"
        ...
        If ($current_IPv4 -ne $new_IPv4)
            {
                Update-Log -logMessage "Error: IPv4 address not set correctly. Script is exiting"
                Exit
            }
        Else
            {
                Update-Log -logMessage "IPv4 address set OK"
                ...
                If ($current_DNS -ne $new_DNS)
                    {
                        Update-Log -logMessage "Error: DNS address not set correctly. Script is exiting"
                        Exit
                    }
                Else
                    {
                        Update-Log -logMessage "DNS address OK"
                    }
            }
    } # Configure-IPandDNS
...
Update-Log -logMessage "First script initialization"
...
Update-Log -logMessage "A restart is required to make these changes. Rerun after logon"
